...
export function Bar(props){
  let {data, interval, height} = props;
  ...
  let positiveHeight = height-50;
  
  data.map((d)=>{
    d.values.map((v)=>{      
      if(v<0){
        positiveHeight = (height-50)/2;
      }
  ...
  return (
    <Svg width="390" height={height}>            
      ...
      {data && data.map((d,i)=>(
        <>
        <Text fill="blue" x={20 + i*50} y={height-40} transform={`rotate(20 ${i*50} ${height-40})`}>{d.label}</Text>
      ...
      <Line x1="0" y1={positiveHeight} x2="390" y2={positiveHeight} stroke="#888888" strokeWidth="1" />      
    </Svg>
  );
}
